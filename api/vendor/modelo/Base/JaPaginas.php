...
abstract class JaPaginas implements ActiveRecordInterface 
{
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : JaPaginasTableMap::translateFieldName('Categoria', TableMap::TYPE_PHPNAME, $indexType)];
            $this->categoria = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : JaPaginasTableMap::translateFieldName('Leermas', TableMap::TYPE_PHPNAME, $indexType)];
            $this->leermas = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : JaPaginasTableMap::translateFieldName('Estado', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getCategoria(),
            $keys[5] => $this->getLeermas(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) {
            $this->setCategoria($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setLeermas($arr[$keys[5]]);
        }
