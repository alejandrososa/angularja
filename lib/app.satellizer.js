...
///(function(window, angular, undefined) {
  ...
  angular.module('satellizer', [])
    .constant('SatellizerConfig', {
      ...
      cordova: false,
      baseUrl: '/',
      loginUrl: '/api/credenciales',
      signupUrl: '/api/registro',
      unlinkUrl: '/auth/unlink/',
      tokenName: 'tokenja',
      tokenPrefix: 'satellizer',
      authHeader: 'autorizacion',
      authToken: 'jajwt',
      storageType: 'localStorage',
      usuario:{nombre:'u_n',id:'u_i'},
      providers: {
        facebook: {
          name: 'facebook',
          url: '/auth/facebook',
    ...
    .provider('$auth', ['SatellizerConfig', function(config) {
      Object.defineProperties(this, {
        usuario: {
          get: function() { return config.usuario; },
          set: function(value) { config.usuario = value; }
        },
        httpInterceptor: {
    ...
    .factory('SatellizerShared', [
      ...
      function($q, $window, config, storage) {
        ...
        var usuario_id = config.usuario.id;
        var usuario_nombre = config.usuario.nombre;
        ...
        Shared.setToken = function(response) {
          ...
          if (!token && response) {
            ...
            u_id = tokenRootData ? tokenRootData[config.usuario.id] : response.data.usuario.id;
            u_nombre = tokenRootData ? tokenRootData[config.usuario.nombre] : response.data.usuario.nombre;
          }
          ...
          storage.set(usuario_id, u_id);
          storage.set(usuario_nombre, u_nombre);
        };

        Shared.removeToken = function() {
          storage.remove(tokenName);
          storage.set(usuario_id);
          storage.set(usuario_nombre);
        };
    ...
    .factory('SatellizerLocal', [
      ...
      'SatellizerShared',
      'SatellizerConfig',
      function($http, utils, shared, config, $log) {
        var Local = {};
    ...
    .factory('SatellizerInterceptor', [
      ...
      '$location',
      '$rootScope',
      function($q,config, storage, shared, $log, $location, $rootScope) {
        return {
          ...
          responseError: function(response) {
            // TODO esto es un parche, ver como no mostrar el error en un futuro
            //location.reload();
